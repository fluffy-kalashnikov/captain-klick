...
void GraphicsDevice::EndFrame(ComPtr<ID3D12GraphicsCommandList>&& aCommandList)
{
    ...
    {
        std::throw_with_nested(std::runtime_error("graphics device failed to end frame"));
    }
}
    ...
    const void* aBuffer, const UINT aBufferByteSize, const UINT aBufferByteStride)
{
    ...
    {
        std::throw_with_nested(std::runtime_error("failed to create default buffer"));
    }
}
...
void GraphicsDevice::WaitForGPU()
{
    ...
    {
        std::throw_with_nested(std::runtime_error("graphics device failed to wait for gpu"));
    }
}